...
/**
  * @brief  This function is used to start FLASH mass erase operation, addr 0x08001000~0x0800FFFF.
  * @retval An ErrorStatus enumeration value:
...
ErrorStatus MassErase(void)
{
	...
	while (dwAddr < dwEndAddr) {
		...
		eResultFlag = SectorErase(&wSectorNum, 0);
		if (SUCCESS != eResultFlag) {
			break;
		}
...
ErrorStatus PageErase(uint16_t* pwDataBuf, uint8_t ucDataLength, uint8_t ucPageCount)
{
  ...
  for (i=0; i<ucDataLength+1; i++)
  {
    ...
    for (j=0; j<ucPageCount; j++)
    {
      ...
      if ((((HW8_REG(FLASHSIZE_BASE)&0x07)>>0)+1)*8*0x400 <= (dwAddr-FLASH_BASE))
      {
        eResultFlag = SUCCESS;
        continue;
      }
...
ErrorStatus SectorErase(uint16_t* pwDataBuf, uint8_t ucDataLength)
{
  ...
  for (i=0; i<ucDataLength+1; i++)
  {
    ...
    //4) 向该 sector 写任意数据
    HW32_REG(FLASH_BASE+pwDataBuf[i]*0x1000) = 0xFFFFFFFF;
